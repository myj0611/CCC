...
4. History - trends in computing
    ...
    - **Grid Computing**
        ...
        - Can be thought of as a distributed system with non-interactive workloads. 分布式系统需要交互，网格计算不需要交互 
...
1. Deployment Models
    ...
    |---|---|---|---|---|
    |pro|1. **Control**<br>2. **Consolidation of resources**<br>3. **Easier to secure** - easy to setup firewall<br>4. **More trust**||1. Utility computing 效用计算，按需收费<br>2. **Can focus on core business** - no need to care infrasture or be a devop（development，operation）应用<br>3. Cost-effective - use as much as you need<br>4. “Right-sizing”<br>5. Democratisation自由度 of computing<br>|1. **Cloud-bursting** - Use private cloud, but burst into (突然变成) public cloud when needed (What is hybrid cloud) |
    |con|1. Relevance to core business?<br>e.g. Netflix to Amazon<br>2. Staff/management overheads - need devop<br>3. Hardware obsolescence过时 - need to refresh hardware<br>4. Over/under utilisation challenges - recycle resources<br>| | 1. **Security** - people can see your sensitive data<br>2. Loss of control<br>3. **Possible lock-in** - difficult to switch Azure if using AWS<br>4. Dependency of Cloud provider continued existence<br>| 1. How do you move data/resources when needed?<br>2. How to decide (in real time?) what data can go to public cloud?<br>3. Is the public cloud compliant with PCI-DSS (Payment Card Industry – Data Security Standard)?<br>|
    |example| | | google doc | Eucalyptus, VMWare vCloud Hybrid Service|

2. Delivery Models
- responsibilities:
    - <img src="./docs/9.jpg" width="70%" height="50%" />
| | Iaas（本身已经有基础设施，用户只需租用，按照使用量付费 | Paas（平台：通过平台去提供服务 |Saas（通过网络提供网络服务|
|---|---|---|---|
| example| Amazon Web Services<br>Oracle Public Cloud<br>NeCTAR，MRC | Azure| Gmail，sharepoint（微博），Microsoft office 365， |
...
- > [2015 Q6] A) Describe the terms Cloud-based IaaS, PaaS and SaaS and give examples for each. [3]
    - IaaS
        - is a computing infrastructure giving access to service where we can now deploy our services on top of that using Ansible/Heater (no need to worry about virtualisation, servers, storage and network)
        - Amazon Web Services
    - PaaS
...
- Applications can be deployed across Clouds either through creation and deployment of virtual images (snapshots) or through scripting the installation and cnfiguration of software applications.
  ...
  1. What are the benefits and drawbacks of these approaches?
...
- > Describe the approach that would be taken using Ansible for scripted deployment of SaaS solutions onto the Cloud. [2] 怎样部署ansible， ansible structure
    
    - create playbook, create inventory file, create roles file, deploy
...
2. Why Service-oriented Architectures - SOA? 面向服务，服务本身就存在在本机上 （benefit）
    - When an architecture is completely contained within the same machine（一个本机上）, components can communicate directly 可以直接访问
        - e.g. through function calls or object instantiations.
    - However, when components are distributed （当分布时）such a direct approach（不能直接访问） typically cannot be used  (e.g. Assignment 2!)
    - Therefore, components (more properly, systems) have to interact in more loosely-coupled ways. （同时实现direct/not direct）
...
4. SOA design principle
    ...
    |Service loose coupling耦合松散| Services maintain a relationship that minimizes dependencies and only requires that they maintain an awareness of each other.||
    ...
    |Service autonomy| Services have control over the logic they encapsulate.|you can have tweets older than 2 weeks than you really can't，只能7天|
    |Service statelessness| Services minimize resource consumption 减少资源消耗 by deferring the management of state information when necessary.||
    |Service discoverability| Services are supplemented with communicative meta data by which they can be effectively discovered and interpreted. 可以有效地被发现和编译，因为是可沟通的meta data||
    |Service composability| Services are effective composition participants有效的组成部分, regardless of the size and complexity of the composition.||
    |Service granularity间隔尺寸| a design consideration to provide optimal scope at the right granular level of the business functionality in a service operation.在正常的大小内提供服务||
...
3. ReST Best Practices (principle)
    ...
    - URIs should be opaque不透明的 identifiers that are meant to be discovered by following hyperlinks, not constructed by the client. （是http开头链接，不是C：/开头
...
6. Resource-Oriented Architecture (ROA)
    ...
    - ROA actions （mapping actions to http methods）CRUD
        ...
        - Don't mapping PUT to Update and POST to create （common mistakes）
            ...
            (Q: what is CRUD means in the HTTP method, how do we use them in RESTful? A: CRUD mean creating, reading, updating, deleting, in the RESTful we can use GET, POST, PUT,DELET to implement those HTTP method)
...
- Discuss the advantages and disadvantages of ReST vs SOAP for web services more generally. [5]

  •      ReST: more about resources, users no need to know what API requires, no need to spend weeks to know input, output parameters functions, much simplified. ReST more a style of HTTP rather than protocol.

  •      SOAP: build on RPC, is a protocol. WSDL provides instructions about a web service, clients send what information they want and the services return the information back. SOAP exposes application logic to public instead of data.

  •      REST Over SOAP:

  REST allows a greater variety of data formats, whereas SOAP only allows XML. REST is generally faster and uses less bandwidth. It’s also easier to integrate with existing websites with no need to refactor site infrastructure.
...
5. CAP Theorem and the Classification of Distributed Processing Algorithms
    ...
    3. Multi-Version Concurrency Control (MVCC): **Availability and Partition-tolerance**
        - MVCC is 
            ...
            - **some sort of recovery from a partition** by reconciling调和 the single databases with revisions (data are not replaced, they are just given a new revision number)
...
2. CouchDB Cluster Architecture
    ...
    - Sharding (splitting of data across nodes) is done on every node
        ...
        - How Shards Look in CouchDB see lecture 13-14 slide 23 （第2点）
            ...
            ```
    - When a node does not contain a document (say, a document of Shard A is requested to Node 2), the node requests it from another node (say, Node 1) and returns it to the client （eg. if a read request to shard A to node 2, then redirect it to node 1 or node 3 answer it
...
- > [2013 Q7] B) What capabilities are currently offered or will be required for Cloud Computing infrastructures such as the NeCTAR Research Cloud to tackle these "big data" challenges. [5]  
  You may refer to specific research disciplines, e.g. life sciences, astrophysics, urban research (or others!) in your answer to part A) and B) of this question.
    ...
    - For life science, there are thousands of databases relevant to bioinformatics and growing! i.e. we know there can be lots of data but we don't know exact how much. So we should make the NeCTAR be scalable to the volumes of the data to be stored.
      ...
      *Securely copy data or transfer: )*
...
2. Function & FaaS
    ...
    - Synchronous/Asynchronous Functions
      ...
      - ●However, there may be functions that take longer to return a result, hence they incur timeouts and lock connections with clients in the process, hence it is better to transform them into asynchronous functions
      - ●Asynchronous  functions  return  a  code  that  informs  the client that the execution has started, and then trigger an event when the execution completes
      - ●In  more  complex  cases  a publish/subscribe   patterninvolving  a  queuing  system  can  be  used  to  deal  with asynchronous functions
...
1. Why we need it?
    - There would not be much point in amassing积累 vast amount of data without being able to analyse it, hence the blossoming of large-scale business intelligence and more complex machine learning algorithms. 
...
3. Examples of Analytics
    ...
    ●**Sentiment analysis** (e.g. deciding whether a tweet on a given topic express a positive or negative emotion)
...
5. Tools for Analytics:
    - Apache Hadoop （the  majority  of applications are built on top of an open-source framework）
    - Apache Spark
### Apache Hadoop
1. How it works?
    ...
    - Apache Hadoop started as a way to distribute files over a cluster and execute MapReduce tasks, but many tools have now been built on that foundation to add further functionality
      ...
      - ●Distributed resource management tools (YARN, Mesos)
      - ●Machine learning libraries (Mahout) 
      - ●Object storage (Ozone): a key-value store optimised for big objects
2. Components
...
2. **Hadoop Distributed File System (HDFS)**
    ...
    - HDFS blocks v.s. blocks
        ...
        - Why? the reasons for this unusual size are：
...
3. **The Hadoop Resource Manager (YARN（Yet Another Resource Negotiator）)** （MapReduce task manager）
...
5. Programming on Hadoop （slide 15 lec17-18）
### Apache Spark
1. Why Spark not Hadoop?/Spark v.s. Hadoop
    ...
    - However,  when  complex  jobs  are  performed  (say, machine learning or graph-based algorithms), there is 
        - a strong incentive for caching data in memory and
        -  in having finer-grained control on the execution of jobs.
...
- > [2014 Q5] B) Apache Hadoop is a software framework that enables processing of large data sets.
    ...
    - > b. Describe the process by which Apache Hadoop supports fault tolerant data processing. [2]
        ...
        -  in our Hadoop framework, HDFS can provide the fault tolerant by apply the replication, replication create a set a copy from the original block, and store the replica in different node, even some node fault we can still have full data.
...
- > [2017 Q2] B What is the Apache Hadoop Resilient Distributed Dataset (RDD) operation type that triggers RDD evaluations? Which operation type does not trigger RDD evaluations? [2]
    ...
    - Spark's RDDs provide two kinds of operations: transformations and actions, where only actions such as reduce or collect trigger the evaluation. So transformation does not trigger RDD evaluations. (Actions trigger RDD evaluations, but transformations not.)
...
A) Code versioning systems are frequently used in collaborative software development activities. Name three types of architectures that code versioning systems have adopted and give one example of a solution for each with their respective advantages and disadvantages. [3] 

Local(Revision Control System):

Centralised (Concurrent Versions System)
...
B) Give a short explanation for the following terms that are often used in a code versioning context: a. Commit [1] b. Checkout [1] c. Branch [1] d. Tag [1] e. Rebase [1] 

**a.     Commit [1]**

**•      write or merge the changes made in the working copy back to the repo**

**b.    Checkout: copy files or optionally switch branches**

**•      create a local working copy from the repo or switch branches.**

**c.     Branch [1]:**

**d.    Tag [1]: a tag or label refers to an important snapshot in time**
...
C) What is the main difference between the clone and checkout commands? [2] 

**•      clone: fetch your repositories from the remote git server.**

**•      checkout: copy file from history to working dir and optionally switch branches**
